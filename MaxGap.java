...
Analysis:
	Method 1: 
		normal way, sort array first and then maintain the max
	...
	Method 2: Bucket Sort
		...
		min or max and there are n-1 buckets, at least one of the buckets are empty. We only need to store the largest 
		number and the smallest number in each bucket.

		After we put all the numbers into the buckets. We can scan the buckets sequentially and get the max gap.
		We only need to compare max number in this bucket and min number in next bucket to get the relatively large 
		gap and find out which two bucket have the largest gap.
*/
