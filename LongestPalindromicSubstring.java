...
1. Dynamic programming solution, O(N^2) time and O(N^2) space:
		Consider the case “ababa”. If we already knew that “bab” is a palindrome, it is obvious that “ababa” must 
		be a palindrome since the two left and right end letters are the same. P[i,j] means substring start from i 
		and end with j is palindrome.
...
2. Based on the solution above, improve it to O(1) space:
		...
		and there are only 2N-1 such centers. (N for center is one character and N-1 for center is between two characters)
*/
...
class LongestPalindromicSubstring {
	...
	//method 3, more easier to understand version, recursive. Time O(n^2)
	int low, maxLen;
