...
In the code, for constant space O(1) we will modify the same 'num' array to store the (small, big, small) hill shape 
values. So the code will not only calculate the length of the sequence but if the interviewers asks for the Wiggle 
series also then we can return the series too. The leetcode Online Judge skipped a test case if the series starts with 
same set of numbers. Thanks to @ztq63830398, modified the code to consider that test case also.
*/
class WiggleSubsequence {
	public int wiggleMaxLength(int[] nums) {
		...
		//skip all the same numbers from beginning
		int k = 0;
		...
		//the result starts from 2
		int result = 2;
		boolean isBigger = nums[k] < nums[k+1];
		for(int i = k + 1; i < nums.length - 1; i++) {
			if(isBigger && nums[i+1] < nums[i]) {
				nums[result] = nums[i+1];
				result++;
				isBigger = !isBigger;
			}
			else if(!isBigger && nums[i+1] > nums[i]) {
				nums[result] = nums[i+1];
				result++;
				isBigger = !isBigger;
			}
