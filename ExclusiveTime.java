    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for(String log : logs) {
            ...
            if(parts[1].equals("start")) {
                if(!stack.isEmpty()) {
                    result[stack.peek()] += Integer.parseInt(parts[2]) - pre;
                }
                stack.push(Integer.parseInt(parts[0]));
                pre = Integer.parseInt(parts[2]);
            }
            else {
                result[stack.pop()] += Integer.parseInt(parts[2]) - pre + 1;
                pre = Integer.parseInt(parts[2]) + 1;
            }
        }
        return result;
    }
